...
class MultiEditDialog extends connect(store)(DialogElement) {
	...
	static override styles = [
		...
		css`
			...
			/* The progress surface itself is the shared <progress-panel>
			   (#758), extracted from the styles that used to live here. */
			.bulk-error progress {
				width: 100%;
	...
	override innerRender() {
		...
		: this._bulkTagProgress
			? html`<progress-panel
							...
							.total=${this._bulkTagProgress.total}
							.value=${this._bulkTagProgress.completed}
							countText='${this._bulkTagProgress.completed} of ${this._bulkTagProgress.total} cards ${this._bulkTagProgress.serverConfirmed ? 'server-confirmed' : 'processed safely'}'
							.detailParagraphs=${[
		this._bulkTagProgress.serverConfirmed

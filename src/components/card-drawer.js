		...
		return html`
			...
			<div ?hidden='${!this.showing}' class='container ${this._dragging ? 'dragging' : ''}${this.reorderPending ? 'reordering':''} ${this.grid ? 'grid' : ''}'>
				...
				${repeat(this.collection, (i) => i.id, (i, index) => html`
					...
					<card-thumbnail @dragstart='${this._handleDragStart}' @dragend='${this._handleDragEnd}' .card=${i} .userMayEdit=${this.editable} .id=${i.id} .name=${i.name} .title=${this._titleForCard(i)} .cardType=${i.card_type} .selected=${i.id == this.selectedCardId} .starred=${this.stars[i.id] || false} .read=${this.reads[i.id] || false} .onReadingList=${this.readingListMap ? this.readingListMap[i.id] : false} .hasTodo=${this.todoMap ? this.todoMap[i.id] : false} .willBeRemovedOnPendingFilterCommit=${this.collectionItemsThatWillBeRemovedOnPendingFilterCommit[i.id] || false}></card-thumbnail>`)}
				</div>

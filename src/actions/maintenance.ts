...
const exportPolymathData : MaintenanceTaskFunction = async (_, getState) => {
	...
	const examples = data.map(record => ({id: record.id, text: record.content, url: origin + urlForCard(record.id), title: record.title, description: record.content.split(' ').slice(0, 100).join(' '), image_url:image}));

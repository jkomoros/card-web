...
- **P1 (fixed): reader multi-tab was nonfunctional — two bugs canceling into "looks fine."** (a) `connectWorkerNow` opened with `if (ownershipState !== 'active') return;`, so a reader session could never spawn a worker (TypeScript had even flagged the dead code — the `as OwnershipState` cast that silenced the narrowing error was the tell). (b) This deployment auto-signs every visitor in ANONYMOUSLY; the readerness predicate required an empty uid, so every visitor "upgraded" to exclusive ownership ~1s after boot — and `flagHasPreviousSignIn()` ran for anonymous sign-ins too, so the marker then routed every subsequent boot straight to exclusive. The validation claim was also illusory: the harness pre-sets the marker in every run, so no automated run had ever taken the reader path. **Fixes:** the guard admits `'reader'`; readerness = `!mayViewUnpublished && (no uid || selectUserIsAnonymous)`; anonymous sign-ins now CLEAR the marker instead of setting it (previously-anonymous devices heal on their first post-deploy visit); and a new `--test-reader` harness scenario boots WITHOUT the marker, anonymous, and asserts cards render in TWO tabs with no gate — the test that would have caught all of it.
- **Aux-write queue (fixed):** the replay watcher captured the first user's uid permanently (sign-out/account-switch would replay the old uid's intents under new auth → permission-denied → silently deleted) — it now reads the live uid from the store; and `not-found`/`failed-precondition` are treated as permanent failures (a starred-then-deleted card would otherwise head-of-line-block the whole queue for 30 days). Verified sound by the audit: aux writes still commit through MultiBatch → they still count in `inFlightMutationCount` for the lease `pending` flag and the SW reload gate; a fenced attempt is retained and replayed elsewhere via shared localStorage.
- **Inert-flag fallback regression (fixed):** `recoverFromWorkerFailure` made the tab network-inert in BOTH branches, and round 5's inert guards then no-op'd the fallback branch's own listener reconnects — a shadow/spike worker failure left the tab with no card listeners at all. The disconnect now runs only in the required-worker branch, and the fallback branch clears any stale inert flag before reconnecting.
- **Sign-out privacy nit (fixed):** card-view's held stale-while-revalidate collection is reset on uid change, so a sign-out can no longer briefly render pre-sign-out unpublished titles under the "updating…" dim.
- **Audit verdicts recorded as accepted:** the suggested-tags first build runs synchronously in the worker (stalls queued pushes once per session; the chunked-build pattern exists if this bites) and its memo rebuilds after every corpus delta (cheap only for per-card fingerprints, re-aggregates tags — acceptable at current tag counts); `createCard` during the boot window fails with console-only feedback (tiny window); concepts arriving mid-edit are picked up one extraction-debounce late; save gating, gate grace (fences verified intact — visual-only), and the rest of the late commits held up under attack.
...
| corpus-mode escape hatch (P3) | **CLOSED for prod** (`diagnosticModesAllowed` forces on+watermark off the allowlisted hosts). Nit: config-specific hostname hardcoded in generic source; `*.firebaseapp.com` dev alias excluded. **DELIBERATELY REOPENED 2026-08-13 (`d7911781`)**: the product audit's decision log (#6) ruled the other way — for a single-admin product, having NO in-browser rollback on prod is the worse hazard, because the only remedy was redeploying master, which strands the durable write queue and drops the edit draft. `diagnosticModesAllowed()` now returns true on every host, so `corpus-worker=off` works on prod. This is not a regression; do not "re-close" it without reading `src/corpus-mode.ts`, which argues the case in place. |

...
### NEXT — small, high-value, still proportionate

4. **Mutation testing** of the guard (`card-write-guard.ts` + the `assertCardWriteBumpsUpdated`/`updateWithoutTimestampBump` logic in `multi_batch.ts`) against the existing core tests — Stryker scoped to those files. Directly measures whether the suite actually *constrains* the guard (flip a `return null`, delete a line → a test must fail). Run: `npx stryker run` with `mutate: ['src/card-write-guard.ts']`. This is the one genuinely-missing technique for a load-bearing pure function; it is cheaper than any harness.
5. **Elevate the choke-point audit + name the server-side boundary.** The bypass audit already runs as a test, but it is heuristic (ref-name matching — its own header says so). Keep it as a tripwire; additionally **document the server-side hole as a first-class named limit** (below), since it is the real residual-risk-#1 exposure.
